local t1 = {}
local function unpackTable(tbl)
    local result = {}
    for i = 1, #tbl do
        result[i] = tbl[i]
    end
    return table.unpack and table.unpack(result) or unpack(result)
end
local v2 = table.unpack or unpack

-- Fallback se não tiver unpack
if not v2 then
    v2 = function(tbl)
        local result = {}
        for i = 1, #tbl do
...
local v10 = true
print("Pulsar Hub iniciado com sucesso!")

local LocalPlayer = game.Players.LocalPlayer
local isOwner = LocalPlayer.Name == "Riftod37"
local s2 = isOwner and "[ Pulsar Hub Owner ]" or "[ Pulsar Hub User ]"

-- Lista de usuários do Pulsar Hub (para o Admin)
t1.value_Users = {}
local function updateUserList()
    t1.value_Users = {}
    for _, player in ipairs(game.Players:GetPlayers()) do
        table.insert(t1.value_Users, player.Name)
    end
end
updateUserList()
...
t1.value19 = t1.value18:MakeWindow({
    Title = "Pulsar Hub",
    SubTitle = isOwner and "Owner: Riftod37" or "Updated - by Dazzy",
    LoadText = "PulsarHub",
    Flags = "PulsarAsWere"
})
...
-- ============================================
-- ABA ADMIN (SÓ PARA O DONO)
-- ============================================
if isOwner then
    local adminTab = t1.value19:MakeTab({
        "Admin",
        "crown"
    })
    
    adminTab:AddSection({ "Painel de Administração" })
    adminTab:AddParagraph({
        "Status:",
        "Você está logado como Owner do Pulsar Hub!"
    })
    
    -- Lista de usuários do Pulsar Hub
    local userListLabel = adminTab:AddParagraph({
        "Usuários Online:",
        "Carregando..."
    })
    
    local function updateAdminUserList()
        local users = {}
        for _, player in ipairs(game.Players:GetPlayers()) do
            table.insert(users, player.Name)
        end
        userListLabel:Set(table.concat(users, ", ") or "Nenhum usuário encontrado")
    end
    
    updateAdminUserList()
    game.Players.PlayerAdded:Connect(updateAdminUserList)
    game.Players.PlayerRemoving:Connect(updateAdminUserList)
    ...
    adminTab:AddButton({
        ...
        Callback = function()
            ...
            t1.value18:Notify({
                Title = "Pulsar Hub",
                Text = "Lista de usuários atualizada!",
                Duration = 3
            })
        end
    })
    
    adminTab:AddSection({ "Anúncios" })
    
    -- Anúncio global (apenas no servidor atual)
    adminTab:AddTextBox({
        Name = "Mensagem do Anúncio",
        PlaceholderText = "Digite sua mensagem aqui...",
        Callback = function(text)
            if text and text ~= "" then
                for _, player in ipairs(game.Players:GetPlayers()) do
                    game:GetService("StarterGui"):SetCore("SendNotification", {
                        Title = "📢 Anúncio do Owner",
                        Text = text,
                        Duration = 8
                    })
                end
                t1.value18:Notify({
                    Title = "Pulsar Hub",
                    Text = "Anúncio enviado para todos os jogadores!",
                    Duration = 3
                })
            end
        end
    ...
    -- Anúncio no chat
    adminTab:AddButton({
        Name = "Enviar Anúncio no Chat",
        Callback = function()
            local text = ""
            -- Pega o texto do TextBox anterior
            -- Como não temos como pegar o valor direto, vamos usar uma variável global
            if _G.AdminAnnounceText and _G.AdminAnnounceText ~= "" then
                for _, player in ipairs(game.Players:GetPlayers()) do
                    if player.Character and player.Character:FindFirstChild("Humanoid") then
                        game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer(
                            "📢 " .. _G.AdminAnnounceText, "All"
                        )
                    end
                end
                t1.value18:Notify({
                    Title = "Pulsar Hub",
                    Text = "Anúncio enviado no chat!",
                    Duration = 3
                })
            else
                t1.value18:Notify({
                    Title = "Pulsar Hub",
                    Text = "Digite uma mensagem no campo acima primeiro!",
                    Duration = 3
                })
            end
        end
    ...
    -- Para salvar o texto do anúncio
    _G.AdminAnnounceText = ""
    adminTab:AddTextBox({
        Name = "Texto do Anúncio",
        PlaceholderText = "Digite o texto do anúncio...",
        Callback = function(text)
            _G.AdminAnnounceText = text
        end
    })
    
    adminTab:AddSection({ "Moderação" })
    ...
    adminTab:AddButton({
        ...
        Callback = function()
            -- Recria o dropdown com a nova lista
            -- Como não temos como atualizar diretamente, vamos notificar
            t1.value18:Notify({
                Title = "Pulsar Hub",
                Text = "Use a função de atualizar acima ou reabra a aba Admin",
                Duration = 3
            })
        end
    })
    
    -- Função para executar ações em jogadores
    adminTab:AddButton({
        Name = "Kill Player",
        Callback = function()
            ...
            if player and player.Character and player.Character:FindFirstChild("Humanoid") then
                ...
                t1.value18:Notify({
                    Title = "Pulsar Hub",
                    Text = "Jogador " .. selectedPlayer .. " foi morto!",
                    Duration = 3
                })
            else
                t1.value18:Notify({
                    Title = "Pulsar Hub",
                    Text = "Jogador não encontrado ou sem character!",
                    Duration = 3
                })
            end
        end
    ...
    adminTab:AddButton({
        ...
        Callback = function()
            ...
            if player and player.Character and player.Character:FindFirstChild("HumanoidRootPart") and localChar and localChar:FindFirstChild("HumanoidRootPart") then
                player.Character.HumanoidRootPart.CFrame = localChar.HumanoidRootPart.CFrame * CFrame.new(0, 0, -5)
                t1.value18:Notify({
                    Title = "Pulsar Hub",
                    Text = "Jogador " .. selectedPlayer .. " foi trazido!",
                    Duration = 3
                })
            else
                t1.value18:Notify({
    ...
    adminTab:AddButton({
        Name = "Kick Player (Teleportar para Void)",
        Callback = function()
            if not selectedPlayer or selectedPlayer == "" then
                t1.value18:Notify({
                    Title = "Pulsar Hub",
            ...
            if player and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                ...
                t1.value18:Notify({
                    Title = "Pulsar Hub",
                    Text = "Jogador " .. selectedPlayer .. " foi teleportado para o void!",
                    Duration = 3
                })
            else
                t1.value18:Notify({
    ...
    adminTab:AddSection({ "Informações do Servidor" })
    
    adminTab:AddParagraph({
        "Jogadores no Servidor:",
        tostring(#game.Players:GetPlayers())
    })
    
    adminTab:AddButton({
        Name = "Atualizar Info do Servidor",
        Callback = function()
            t1.value18:Notify({
                Title = "Pulsar Hub",
                Text = "Jogadores no servidor: " .. tostring(#game.Players:GetPlayers()),
                Duration = 3
            })
        end
    })
